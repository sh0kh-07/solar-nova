...
export const Ombor: React.FC = () => {
  ...
  // Low stock counts
  const rawLowStockCount = rawMaterials.filter(rm => rm.currentStock < rm.minStock).length;
  const finishedLowStockCount = finishedProducts.filter(fp => fp.stock < fp.minStock).length;
  ...
  const filteredRaw = useMemo(() => {
    return rawMaterials.filter(rm => {
      ...
      return rm.name.toLowerCase().includes(q) || rm.code.toLowerCase().includes(q);
    });
  ...
  const filteredFinished = useMemo(() => {
    return finishedProducts.filter(fp => {
      ...
      return (
        ...
        fp.type.toLowerCase().includes(q)
      );
    });
  ...
  const filteredMovements = useMemo(() => {
    return warehouseMovements.filter(wm => {
      ...
      const q = searchQuery.toLowerCase();
      return (
        wm.itemName.toLowerCase().includes(q) ||
        wm.performedBy.toLowerCase().includes(q) ||
        wm.notes.toLowerCase().includes(q)
      );
    });
  }, [warehouseMovements, searchQuery]);
  ...
  const handleSaveMovement = (e: React.FormEvent) => {
    ...
    recordWarehouseMovement({
      type: movementType,
      itemType: selectedTargetType === 'raw' ? 'raw_material' : 'finished_product',
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white p-2 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-2 overflow-x-auto">
        ...
        >
          ...
          <span>2. Tayyor mahsulot (Ustunlar)</span>
        ...
        >
          ...
          <span>3. Harakatlar (Kirim / Chiqim)</span>
        </button>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 text-[11px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                  ...
                  <th className="py-3.5 px-4">Balandligi (m)</th>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 text-[11px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                  ...
                  <th className="py-3.5 px-4">Mas’ul shaxs</th>
                  <th className="py-3.5 px-4">Izoh / Hujjat</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-sm">
                {filteredMovements.map(mov => (
                  <tr key={mov.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-3.5 px-4 sm:px-6 whitespace-nowrap">
                      ...
                      <div className="text-[11px] text-slate-400">{mov.time}</div>
                    </td>
                    <td className="py-3.5 px-4">
                      <span
